...
sub register {
  ...
  state $script = $plugin_param->{script} ? $plugin_param->{script} : 'matomo';
    ...
    piwik_tag => sub {
      ...
      if ($as_script) {
        ...
        return b('<script src="' . $c->url_for('piwik_track_script')->to_abs . '"></script>' .
                   "<script src=\"$prot://${url}${script}.js\" async defer></script>");
      }
...
_paq.push(['setTrackerUrl',u+'${script}.php']);_paq.push(['setSiteId',${site_id}]);
var d=document,g=d.createElement('script'),s=d.getElementsByTagName('script')[0];
g.type='text/javascript';g.async=true; g.src=u+'${script}.js'; s.parentNode.insertBefore(g,s);
})();$append</script>
<noscript><img src="${prot}://${url}${script}.php?idSite=${site_id}&amp;rec=1" alt="" style="border:0"/></noscript>
SCRIPTTAG
    });
          ...
          cb => sub {
            ...
            return $c->render(
              ...
              text => 'var _paq=window._paq=window._paq||[];' .
                "_paq.push(['setTrackerUrl','$prot://${url}${script}.php']);" .
...
C<append> - Additional tracking code, that is appended to the code
generated by L</piwik_tag> and the C<track_script> of the piwik shortcut.
B<Experimental!>

=item

C<script> - The name of the tracking script. Defaults to C<matomo>
(ending with C<.js> or C<.php>), but was C<piwik> before.
